package Exam_Advance_1.presenatation;
...
import Exam_Advance_1.business.design.features.implement.CartFeatureImpl;
import Exam_Advance_1.business.design.features.implement.ProductFeatureImpl;
import Exam_Advance_1.units.Color;
...
public class UserManagement {
    ...
    private void showAllCart() {
        if (CartFeatureImpl.cartItems.isEmpty()){
            System.out.println("Danh sách CartItem trống không thể hiển thị ra màn hình");
            return;
        }
    ...
    private void addToCart(Scanner sc) {
        if (ProductFeatureImpl.products.isEmpty()){
            System.err.println("Danh sách sản phẩm trống không thể cho vào giỏ hàng");
            return;
        }
    ...
    private void displayAllProduct() {
        if (ProductFeatureImpl.products.isEmpty()){
            System.err.println("Dánh sách sản phẩm trống");
            return;
        }
        int count=0;
        for (Product product: ProductFeatureImpl.products){
        ...
        System.out.println(Color.GREEN+"Cửa hàng có "+count+" sản phẩm"+Color.RESET);
    }
}
